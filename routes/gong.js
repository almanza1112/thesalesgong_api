...
router.post("/hit", async (req, res) => {
  ...
  firestore
    ...
    .get()
    .then((doc) => {
      // Check if team member is in the team
      let uid_team_members = doc.data().uid_team_members;
      if (!uid_team_members.includes(gongSenderUid)) {
        console.log(2)

        // User is not in the team
        res.status(400).json({ message: "User is not in the team." });
      } else {
